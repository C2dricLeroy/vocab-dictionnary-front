...
export const DashboardDictionaries: React.FC = () => {
    ...
    const { data: dictionaries = [], isLoading, isError, refetch } = useDictionaries(session);
    ...
    return (
        <>
            ...
            <Card className="bg-white dark:bg-gray-800 h-full">
                ...
                <CardContent>
                    <div className="text-gray-800 dark:text-white space-y-4">
                        <p>
                            {t("Total dictionaries")}: <span className="font-semibold">{dictionaries.length}</span>
                        </p>
            ...
            <AddDictionaryModal open={isModalOpen} onClose={closeModal} onSubmit={handleModalSubmit} />
        </>
    );
};
